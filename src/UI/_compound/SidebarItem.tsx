...
export default function SidebarItem({ item }: Props) {
    ...
    return (
        <div
            className={`flex items-center w-full h-12 px-3 mt-2 rounded hover:text-gray-950 dark:hover:text-gray-50 text hover:bg-emerald-500 relative`}
        >
            {Icono({ ico: item.ico })}
            <Button
            ...
            />
            {
                item.chils && active && <div className={`${active ? `scale-100 top-10` : `scale-0 top-16`} py-3 grid duration-300 absolute w-full bg-gray-50 dark:bg-slate-700 shadow top-10 z-[5] scale-x-110 left-5 rounded`}>
                    {
